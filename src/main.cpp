...
SemaphoreHandle_t sseSendMutex = NULL;
...
String processor(const String &var)
{
	...
	// Legacy-Platzhalter (nicht mehr im Bridge-UI verwendet)
	else if (var == "SUNR" || var == "SUNS") { return "--:--"; }
	else if (var == "SUNRISE_HTML_TIME" || var == "SUNSET_HTML_TIME") { return "ausSun"; }
	else if (var == "SUNRISEOFFSET" || var == "SUNSETOFFSET") { return "0"; }
	else if (var == "KOOR_LATITUDE" || var == "KOOR_LONGITUDE") { return ""; }
	else if (var.startsWith("TOGBUT")) { return ""; }
	else if (var.startsWith("TOGGLE_BUTTON_")) { return "red"; }
	else if (var == "FS_FLIST" || var == "PINSTA") { return ""; }
	else if (var == "fp_scanner_chk") { return ""; }
	else if (var == "FP_SECTION_CLASS") { return "fp-section-hidden"; }
	...
	else if (var == "fp_scanner_chk" || var == "FP_SECTION_CLASS") { return ""; }
	else if (var == "SUNRISEOFFSET" || var == "SUNSETOFFSET") { return "0"; }
...
doc["mqtt_status"] = mqttStatusString;
doc["mqtt_root_topic"] = app.mqttRootTopic;
#else
doc["mqtt_mode"] = "off";
...
{
noteHttpActivity(request);
if (ENABLE_PASSWORD == "on" && !loggedIn)
{
...
triggerNum = request->getParam("n", true)->value().toInt();
if (triggerNum < 1 || triggerNum > 99)
{
request->send(400, "application/json", "{\"error\":\"invalid trigger number\"}");
return;
}
#if USE_MQTT_CLIENT
bridgeSendTrigger((uint8_t)triggerNum);
#endif
LOG_PRINTF("[Bridge] Trigger %d ausgelöst\n", triggerNum);
notifyClients("Bridge-Trigger " + String(triggerNum) + " gesendet", "[BRIDGE]");
request->send(200, "application/json", "{\"ok\":true,\"trigger\":" + String(triggerNum) + "}");
});
...
for (int i = 0; i < logMessagesCount; i++)
{
int idx = (logNextIndex + i) % logMessagesCount;
if (!logMessages[idx].isEmpty())
arr.add(logMessages[idx]);
}
String out;
serializeJson(doc, out);
AsyncWebServerResponse *response = request->beginResponse(200, "application/json", out);
...
void handleMqttMessage(const String &topic, const String &fullPayload, const String &source, bool retained)
{
	...
	else if (command == "trigger" || topic.endsWith("/trigger"))
	{
		// Bridge-Trigger empfangen (z.B. von anderem Gerät)
		LOG_PRINTLN(logPrefix + " Bridge-Trigger empfangen: " + payload);
		notifyClients("Bridge-Trigger: " + payload, sourceTag.c_str());
	}
	else
	{
		// Unbekannte Befehle still ignorieren
	}
}
#endif // USE_MQTT_ANY
...
void setup()
{
	...
	addLogMessage("Systemstart: MatterMQTTBridge initialisiert.");
	addLogMessage("Firmware-Version: " + String(firmwareVersion));

#if LOGGING
	// Nach Serial.begin(...)
	LOG_PRINTLN();
	LOG_PRINTLN("==================== MatterMQTTBridge PARIP69 ====================");
	...
	if (currentMode != Mode::wificonfig)
	{
		...
		LOG_PRINTLN("╚══════════════════════════════════════════════════════════════╝");
		LOG_PRINTLN();
		LOG_PRINTLN("🚀 MatterMQTTBridge bereit!");
	}
	else
	{
		LOG_PRINTLN("🔧 WiFi-Konfigurationsmodus aktiv");
...
void loop()
{
	...
	if (ENABLE_PASSWORD == "on")
	{
		if (loggedIn)
		{
			if (millis() - lastActivityTime > 600000) // 10 min
			{
				loggedIn = false;
