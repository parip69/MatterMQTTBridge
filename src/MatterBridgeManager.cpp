...
void MatterBridgeManager::begin() {
    ...
    gOnOffLight.onChange([](bool state) -> bool {
        if (state) {
            addLogMessage("Matter ON erhalten -> Trigger 1");
            if (!bridgeSendTrigger(1)) {
                addLogMessage("Matter Fehler: Trigger 1 konnte nicht gesendet werden");
                return false;
            }
            return true;
        }

        addLogMessage("Matter OFF erhalten -> Trigger 2");
        if (!bridgeSendTrigger(2)) {
            addLogMessage("Matter Fehler: Trigger 2 konnte nicht gesendet werden");
            return false;
        }
        return true;
    });
